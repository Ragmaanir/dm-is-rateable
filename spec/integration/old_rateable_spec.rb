  ...
  describe DataMapper::Is::Rateable do
    ...
    shared_examples "every rating" do
      ...
      it "should define a remixed model that can be auto_migrated" do
        ...
        Object.const_defined?("TripUserRating").should be_true
        lambda { TripUserRating.auto_migrate! }.should_not raise_error
      end
      ...
      it "should define a 'rateable_model' class_level reader on the remixing model" do
        ...
        Trip.rateable_model.should == "TripUserRating"
      end
      ...
      it "should define a 'rateable_key' class_level reader on the remixing model" do
        ...
        Trip.rateable_key.should == :trip_user_rating
      end
      ...
      it "should define a 'total_rating' class_level reader on the remixed rateable model" do
        ...
        TripUserRating.total_rating.should == 0
    ...
    shared_examples "every enabled personalized rating" do
      ...
      it "should allow to access any user's current remixed rating model instance" do
        ...
        @t1.user_rating(@u1).should == TripUserRating.get(1)
        @t1.user_rating(@u2).should be_nil
        @t1.rate(5, @u2)
        #@t1.user_rating(@u1).should == TripRating.get(1)
        #@t1.user_rating(@u2).should == TripRating.get(2)
        @t1.user_rating(@u1).should == TripUserRating.get(1)
        @t1.user_rating(@u2).should == TripUserRating.get(2)
      end
      ...
      it "should store exactly one rating at any time for any given user" do
        ...
        TripUserRating.count.should == 1
        @t1.rate(3, @u2)
        @t1.rate(5, @u2)
        #TripRating.count.should == 2
        TripUserRating.count.should == 2
      end
      ...
      it "should allow any user to rate multiple times without compromising the total_rating" do
        ...
        TripUserRating.total_rating.should == 4
        ...
        TripUserRating.total_rating.should == 2
    ...
    describe "Trip.is(:rateable, :rater => :user)" do
      ...
      before do
        ...
        unload_consts('Trip','User','TripUserRating')
    ...
    describe "Trip.is(:rateable, :rater => :user, :as => :my_trip_ratings) without additional properties" do
      ...
      before do
        ...
        class Trip
          ...
          is :rateable, :by => :users, :as => :my_trip_ratings
        end
    ...
    describe "Trip.is(:rateable, :rater => :user, :timestamps => false) without additional properties" do
      ...
      before do
        ...
        class Trip
          ...
          is :rateable, :by => :users, :timestamps => false
        end
    ...
    describe "Trip.is(:rateable, :rater => :account) without additional properties" do
      ...
      before do
        ...
        #unload_rating_infrastructure "Trip", "User"
        unload_consts('Trip','Account','TripAccountRating')
        ...
        Trip.auto_migrate!
        #TripRating.auto_migrate!
        TripAccountRating.auto_migrate!
    ...
    # --------------------------------------------------------------------------------------------------
    # --------------------------------------------------------------------------------------------------
    describe 'is multi-rateable' do
      before do
        #unload_rating_infrastructure 'Trip', 'User'
        #unload_rating_infrastructure 'Trip', 'Party'
        unload_consts 'UserTripRating','PartyTripRating','Trip','Party','User'
        ...
        class Trip
          ...
          is :rateable, :as => :user_ratings, :by => :users
          is :rateable, :as => :party_ratings, :by => :parties
          
          enhance :rating do
            def enhanced_method; id; end
          end
        end
        ...
        Party.auto_migrate!
        Trip.auto_migrate!
        #UserTripRating.auto_migrate!
        #PartyTripRating.auto_migrate!
        TripUserRating.auto_migrate!
        TripPartyRating.auto_migrate!
      end
      ...
      context 'the rateable' do
        subject{ trip }
        it{ should respond_to(:user_ratings) }
        it{ should respond_to(:party_ratings) }
        its(:user_ratings){ should_not == trip.party_ratings }
        it 'has two ratings remixed' do
          Trip.remixables[:rating].should have(2).keys
        end
      end
      
      context 'saving a user rating' do
        let(:user_rating) { UserTripRating.new(:trip => trip, :user => user, :rating => 1) }
        subject{ user_rating.save }
        it{ should be_true }
        it{ expect{ subject }.to change{UserTripRating.count}.by(1) }
      end

      shared_examples :enhanced_ratings do
        let!(:rating){ subject }
        
        it 'has method :enhanced_method' do
          should respond_to(:enhanced_method)
        end
        its(:enhanced_method){ should == rating.id }
      end

      context 'user rates' do
        subject{ trip.rate(2,user) }
        it 'creates a new record' do
          expect{ subject }.to change{UserTripRating.count}.by(1)
        end
